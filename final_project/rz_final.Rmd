...
corr_data<- select_if(train_set,is.numeric) %>%
  select(-Id) %>%
  as.matrix(.) %>%
  rcorr(.)
...
x <- flattenCorrMatrix(corr_data$r, corr_data$P) %>%
  arrange(desc(abs(cor))) %>%
...
ggplot(test_set, aes(x = GrLivArea, y = SalePrice)) +
  geom_point() + 
  geom_smooth() +
...
model4 <- train_set %>%
  ...
  lm(log(SalePrice) ~ GrLivArea + TotalBsmtSF + OverallQual + Neighborhood + NewHome +
       Age + CentralAir + Fireplaces + GarageArea + TotalBath  + PorchSqFt + PoolArea +
       SaleCondition + MSZoning + BldgType + OverallCond , data = .)
summary(model4)
```
...
we selected model 1 due to it fitting the data well (R^2 = 0.8802) and its only slightly worse than model 3 
(R%2 = 0.8813), which is more computationally expensive. We think model 1 would be a better fit for a production environment where such a model might be used, to predict housing prices on demand, as it is a great fit and computes reasonably fast 
...
test.impute = test%>%
  na_replace()%>%
  mice_imputation
prices.test$SalePrice = predict(model1, test.impute)
head(prices.test)
```
